...
class BitRateController:
    def __init__(self, height: int, width: int, config: CodecConfig) -> None:        
        ...
        self.i_frame_bit_count = [(bc, qp) for qp, bc in self.config.RCTable['I'].items()]
        self.p_frame_bit_count = [(bc, qp) for qp, bc in self.config.RCTable['P'].items()]
    ...
    def _find_closest_qp(self, budget: int, is_i_frame: int) -> int:
        ...
        index = bisect.bisect_left(bit_count, budget)
        if index == 0:
    ...
    def get_quant_level(self, is_i_frame) -> int:        
        if self.config.RCflag == 0:
            return self.config.quant_level
