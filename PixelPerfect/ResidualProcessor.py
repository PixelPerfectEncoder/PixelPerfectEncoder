...
class ResidualProcessor:
    def __init__(self, block_size, max_val=255, quanti_level = 2,):
        ...
        elif quanti_level > math.log(block_size+7,2):
            raise Exception("Error! the quanti_level is too large")
        for iy, ix in np.ndindex(self.quant_matrix.shape):
            if (ix + iy) < block_size-1:
                self.quant_matrix[iy][ix] = math.pow(2, quanti_level)
            elif (ix + iy) == block_size-1:
                self.quant_matrix[iy][ix] = math.pow(2, quanti_level+1)
            else:
                self.quant_matrix[iy][ix] = math.pow(2, quanti_level + 2)
